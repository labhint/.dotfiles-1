...
development_go() {
	docker run -ti --rm --privileged -v $(which docker):/usr/local/bin/docker -v /var/run/docker.sock:/var/run/docker.sock -v $HOME/development/:/root/development/go/src/github.com/pandrew/ -v ~/.ssh:/root/.ssh "$@" pandrew/.dotfiles
}
# Usage: puniq [path]
